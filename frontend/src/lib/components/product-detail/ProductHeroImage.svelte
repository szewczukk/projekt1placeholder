...
<div class="relative overflow-hidden rounded-3xl bg-white/70 p-3 shadow-2xl ring-1 ring-white/60 backdrop-blur">
	{#if hasError}
		<div
	...
	{#if isLoading && !hasError}
		<div class="absolute inset-3 overflow-hidden rounded-2xl bg-slate-200">
			<div class="h-full w-full animate-pulse bg-gradient-to-r from-slate-200 via-slate-100 to-slate-200"></div>
		</div>
	{/if}

	<div class="pointer-events-none absolute inset-x-3 bottom-3 rounded-2xl bg-gradient-to-t from-black/50 to-transparent p-6">
